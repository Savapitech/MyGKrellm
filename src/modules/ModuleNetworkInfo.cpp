...
void ModuleNetworkInfo::draw(IDisplay &display) {
  ...
  for (auto &inet : inets) {
    ...
    display.drawText(x + 1, y += 2, "\tReceive: " + std::to_string(inet.getRBits()) + " B, " + std::to_string(inet.getRPackets()) + " packet(s),");
    display.drawText(x + 1, y += 2, "\t\t" + std::to_string(inet.getRError()) + " error(s), " + std::to_string(inet.getRDrop()) + " drop(s).");
    display.drawText(x + 1, y += 2, "\tTransmit: " + std::to_string(inet.getTBits()) + " B, " + std::to_string(inet.getTPackets()) + " packet(s),");
